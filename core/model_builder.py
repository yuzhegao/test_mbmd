...
def build_man_model(model_config, is_training):
    """
    该函数的作用应该是将　config文件中的 网络模型的配置　转化为函数(或object)输入函数　MANMetaArch
    """
    ...
    region_similarity_calculator = sim_calc.build(model_config.similarity_calculator)
    ...
    non_max_suppression_fn, score_conversion_fn = post_processing_builder.build(model_config.post_processing)
    (classification_loss,
     localization_loss,
